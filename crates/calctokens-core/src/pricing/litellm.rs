        ...
        match client.get(PRICING_URL).send().await {
            Ok(response) => {
                ...
                if status.is_server_error() || status == reqwest::StatusCode::TOO_MANY_REQUESTS {
                    eprintln!(
                        "[calctokens] LiteLLM HTTP {} (attempt {}/{})",
                        status,
                        attempt + 1,
                        MAX_RETRIES
                    );
                ...
                if !status.is_success() {
                    eprintln!("[calctokens] LiteLLM HTTP {}", status);
                ...
                match response.json::<PricingDataset>().await {
                    Ok(data) => {
                        if let Err(e) = cache::save_cache(CACHE_FILENAME, &data) {
                            eprintln!(
                                "[calctokens] Warning: Failed to cache LiteLLM pricing at {}: {}",
                    ...
                    Err(e) => {
                        eprintln!("[calctokens] LiteLLM JSON parse failed: {}", e);
                        return Err(e);
                    }
                }
            }
            Err(e) => {
                eprintln!(
                    "[calctokens] LiteLLM network error (attempt {}/{}): {}",
                    attempt + 1,
                    MAX_RETRIES,
                    e
                );
