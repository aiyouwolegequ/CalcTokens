    ...
    pub fn resolve_with_env_strategy(&self, home_dir: &str, use_env_roots: bool) -> String {
        match self {
            ...
            PathRoot::Config => {
                ...
                // `{home}/.config/calctokens` everywhere would diverge from
                // dirs::config_dir() on Windows (where it resolves to
                // %APPDATA%\calctokens), causing synced data to land in
                // %APPDATA% while the scanner looks in %USERPROFILE%.
                #[cfg(target_os = "windows")]
                {
                    if let Some(dir) = dirs::config_dir() {
                        return dir.join("calctokens").to_string_lossy().into_owned();
                    }
                }

                format!("{home_dir}/.config/calctokens")
            }
            PathRoot::EnvVar {
                var,
                fallback_relative,
...
define_clients!(
    ...
    Cursor = 3 => {
        id: "cursor",
        root: PathRoot::Home,
        relative: ".config/calctokens/cursor-cache",
        pattern: "usage*.csv",
        headless: false,
        parse_local: false,
        submit_default: true
...
mod tests {
    ...
    fn test_path_root_config_uses_override_when_set() {
        ...
        unsafe {
            std::env::set_var("CALCTOKENS_CONFIG_DIR", "/tmp/custom-config-root");
    ...
    fn test_path_root_config_uses_dirs_config_dir_on_windows() {
        ...
        // i.e. dirs::config_dir() (= %APPDATA%\calctokens). Hardcoding
        // {home}/.config/calctokens would diverge from the writer side
        ...
        assert_eq!(
            resolved, expected,
            "PathRoot::Config on Windows must match dirs::config_dir().join('calctokens') so the scanner agrees with the writer"
        );
