...
fn legacy_cache_paths() -> Vec<PathBuf> {
    ...
    [
        ...
        crate::paths::legacy_dot_cache_calctokens_dir().map(|d| d.join(CACHE_FILENAME)),
    ]
    .into_iter()
    .flatten()
    .collect()
...
fn user_scoped_temp_dir() -> Option<PathBuf> {
    ...
    Some(std::env::temp_dir().join(format!("calctokens-uid-{uid}")))
}
...
fn user_scoped_temp_dir() -> Option<PathBuf> {
    std::env::var_os("USERNAME")
        ...
        .map(|user| {
            ...
            path.push(format!("calctokens-user-{}", user.to_string_lossy()));
            path
        })
}
...
mod tests {
    ...
    fn test_fallback_cache_dir_prefers_runtime_dir() {
...

        {
            assert_eq!(
                fallback_cache_dir(),
                Some(runtime_dir.path().join("calctokens"))
            );
        }
    ...
    fn load_falls_back_to_legacy_dirs_cache_path() {
        ...
        restore_env_var("CALCTOKENS_CONFIG_DIR", original_override);
    }
    ...
    fn load_falls_back_to_legacy_dot_cache_path() {
        ...
        let legacy_path = crate::paths::legacy_dot_cache_calctokens_dir()
            .unwrap()
            .join(CACHE_FILENAME);
        ensure_cache_dir(legacy_path.parent().unwrap()).unwrap();
        let store = CachedSourceStore {
