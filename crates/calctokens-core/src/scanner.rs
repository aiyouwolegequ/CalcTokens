...
/// like `CALCTOKENS_EXTRA_DIRS` — it lives on the `scanner` key inside
/// `~/.config/calctokens/settings.json` and is threaded down into
...
pub struct ScannerSettings {
    ...
    /// data directory, so calctokens's auto-discovery can't find it.
    ///
...
pub fn headless_roots_with_env_strategy(home_dir: &str, use_env_roots: bool) -> Vec<PathBuf> {
    if use_env_roots {
        if let Ok(path) = std::env::var("CALCTOKENS_HEADLESS_DIR") {
    ...
    roots.push(PathBuf::from(format!(
        "{}/.config/calctokens/headless",
        home_dir
    )));

    let mac_root = PathBuf::from(format!(
        "{}/Library/Application Support/calctokens/headless",
        home_dir
    ));
    roots.push(mac_root);
...
/// settings (e.g. from `~/.config/calctokens/settings.json`). Thin wrappers
...
fn scan_all_clients_with_env_strategy_inner(
    ...
    if use_env_roots {
        let extra_dirs_val = std::env::var("CALCTOKENS_EXTRA_DIRS").unwrap_or_default();
    ...
    if enabled.contains(&ClientId::OpenCode) {
        ...
        // `calctokens --client claude` does not pull in OpenCode dbs the user pinned
...
mod tests {
    ...
    fn test_headless_roots_default() {
        let previous = std::env::var("CALCTOKENS_HEADLESS_DIR").ok();
        unsafe { std::env::remove_var("CALCTOKENS_HEADLESS_DIR") };

        let home = "/tmp/calctokens-test-home";
        let roots = headless_roots(home);
        let config_root = PathBuf::from(format!("{}/.config/calctokens/headless", home));
        let mac_root = PathBuf::from(format!(
            "{}/Library/Application Support/calctokens/headless",
            home
        ));
        ...
        restore_env("CALCTOKENS_HEADLESS_DIR", previous);
    }
    ...
    fn test_headless_roots_override() {
        let previous = std::env::var("CALCTOKENS_HEADLESS_DIR").ok();
        unsafe { std::env::set_var("CALCTOKENS_HEADLESS_DIR", "/custom/headless") };
    ...
    fn test_headless_roots_ignore_env_override_when_disabled() {
        ...
        assert_eq!(
            roots,
            vec![
                PathBuf::from("/tmp/home/.config/calctokens/headless"),
                PathBuf::from("/tmp/home/Library/Application Support/calctokens/headless")
            ]
        );
    ...
    fn test_scan_all_clients_with_scanner_settings_dedups_settings_and_env_extra_paths() {
        let previous = std::env::var("CALCTOKENS_EXTRA_DIRS").ok();
        let dir = TempDir::new().unwrap();
        let home = dir.path();
        ...
        assert_eq!(result.get(ClientId::Codex).len(), 2);
        restore_env("CALCTOKENS_EXTRA_DIRS", previous);
    }
    ...
    fn test_scan_all_clients_with_scanner_settings_respects_opencode_client_filter() {
        ...
        // guard. A request like `calctokens --client claude` would still pull in
  ...
  "projects": [
    ...
    fn test_scan_all_clients_headless_paths() {
        let previous_headless = std::env::var("CALCTOKENS_HEADLESS_DIR").ok();
        unsafe { std::env::remove_var("CALCTOKENS_HEADLESS_DIR") };
        ...
        restore_env("CALCTOKENS_HEADLESS_DIR", previous_headless);
    }
